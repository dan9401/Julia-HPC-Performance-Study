...
function eqPriceGenp(lsmc::LsmcSpec, rng)
    option = lsmc.option
    S, r, σ= option.S, option.r, option.σ
    numSteps, numSims, δ_t = lsmc.numSteps, lsmc.numSims, lsmc.δ_t
    S_t = zeros(numSteps + 1)
    ...
    @inbounds for i in 1:numSteps
        S_t[i+1] = S_t[i] * mult * exp(σ * sqrt(δ_t) * randn(rng))
    end
...
function priceGridp(lsmc::LsmcSpec, rngs)
    S, K = lsmc.option.S, lsmc.option.K
    numSteps, numSims = lsmc.numSteps, lsmc.numSims
    sGrid = poGrid = zeros(numSims, numSteps+1)

    @inbounds Threads.@threads for i in 1:numSims
        sGrid[i,:] = eqPriceGenp(lsmc, rngs[Threads.threadid()])
    end
